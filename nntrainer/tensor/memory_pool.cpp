...
void MemoryPool::allocateInternal(const size_t alignment) {
  ...
  std::cout << "system alignment: " << GET_SYSTEM_ALIGMENT() << std::endl;
  if (pool_size_ == 0) {
  ...
  for (auto &s : memory_offset_) {
    ...
    if (it == allocated_ptrs_.end()) {
      void *ptr = allocBytes(current_size, GET_SYSTEM_ALIGMENT());
      ...
      if (max_size < current_size) {
        void *new_ptr = allocBytes(current_size, GET_SYSTEM_ALIGMENT());
