...
static void rmsnorm_67_3072(const nntrainer::TensorDim::DataType data_type) {
  ...
  nntrainer::TensorDim::TensorType t_type_nchw_fp16 = {nntrainer::Tformat::NCHW,
                                                       data_type};

  nntrainer::Tensor in_fp16(batch, channel, height, width, t_type_nchw_fp16);
  nntrainer::Tensor gamma_fp16(1, 1, 1, width, t_type_nchw_fp16);
  nntrainer::Tensor out_cl_fp16(batch, channel, height, width,
                                t_type_nchw_fp16);
  nntrainer::Tensor out_ref_fp16(batch, channel, height, width,
                                 t_type_nchw_fp16);

  /// Initialize CPU input data
  GEN_TEST_INPUT(in_fp16, ((i * (batch * height * channel) +
                            j * (batch * height) + k * (width) + l + 1) %
                           MOD) *
                            alpha);
  for (int l = 0; l < width; ++l) {
    float val = ((l + 1) % MOD) * alpha;
    gamma_fp16.setValue(0, 0, 0, l, val);
  }
  ...
  void *in_fp16_svm = allocateSVM(in_fp16.size() * sizeof(T));
  void *gamma_fp16_svm = allocateSVM(gamma_fp16.size() * sizeof(T));
  void *out_fp16_svm = allocateSVM(out_cl_fp16.size() * sizeof(T));

  blas_cc->command_queue_inst_.enqueueSVMMap(in_fp16_svm,
                                             in_fp16.size() * sizeof(T), false);
  blas_cc->command_queue_inst_.enqueueSVMMap(
    gamma_fp16_svm, gamma_fp16.size() * sizeof(T), false);

  std::memcpy(in_fp16_svm, in_fp16.getData<T>(), in_fp16.size() * sizeof(T));
  std::memcpy(gamma_fp16_svm, gamma_fp16.getData<T>(),
              gamma_fp16.size() * sizeof(T));

  blas_cc->command_queue_inst_.enqueueSVMUnmap(in_fp16_svm);
  blas_cc->command_queue_inst_.enqueueSVMUnmap(gamma_fp16_svm);
  ...
  for (unsigned int i = 0; i < run_count; ++i) {
    rmsnorm_cl((T *)in_fp16_svm, (T *)gamma_fp16_svm, (T *)out_fp16_svm,
               kEpsilon, in_fp16.batch() * in_fp16.channel() * in_fp16.height(),
               in_fp16.width(), true);
  }
  ...
  for (unsigned int i = 0; i < run_count; ++i) {
    std::function<T(T)> f = [](T x) { return 1 / std::sqrt(x); };
    auto t = in_fp16.multiply(in_fp16).average(3).add(kEpsilon);
    t.apply_i(f);
    in_fp16.multiply(t, out_ref_fp16);
    out_ref_fp16.multiply_i(gamma_fp16);
  }
  ...
  blas_cc->command_queue_inst_.enqueueSVMMap(
    out_fp16_svm, out_cl_fp16.size() * sizeof(T), false);

  float mseError =
    mse<T>((T *)out_fp16_svm, out_ref_fp16.getData<T>(), height * width);

  double cosSim = cosine_similarity<T>(
    (T *)out_fp16_svm, out_ref_fp16.getData<T>(), height * width);

  const float epsilon = 1e-3;

  freeSVM(out_fp16_svm);
  freeSVM(in_fp16_svm);
  freeSVM(gamma_fp16_svm);
